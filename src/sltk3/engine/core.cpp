...
namespace Sltk{
	namespace Engine{
		...
		bool registerCoreLibrary(lua_State *L){
			...
			// Sltk::LibraryLoader::getFunction
			...
			tolua_lockclass(L); // -1:Sltk
			// Sltk::File::write,read
			...
			tolua_lockclass(L); // -1:Sltk
			// Sltk::Engine::loadString, require
			...
			if(0 != luaL_loadbuffer(L, (const char*)table_lib_code, sizeof(table_lib_code), "<SeleneTalk/Define/Table>")
				|| 0 != lua_pcall(L, 0, 0, 0)) return false;
			if(0 != luaL_loadbuffer(L, (const char*)string_lib_code, sizeof(string_lib_code), "<SeleneTalk/Define/String>")
				|| 0 != lua_pcall(L, 0, 0, 0)) return false;
			if(0 != luaL_loadbuffer(L, (const char*)lpeg_lib_code, sizeof(lpeg_lib_code), "<SeleneTalk/Define/LPeg>")
				|| 0 != lua_pcall(L, 0, 0, 0)) return false;
			if(0 != luaL_loadbuffer(L, (const char*)main_lib_code, sizeof(main_lib_code), "<SeleneTalk/Define/Main>")
				|| 0 != lua_pcall(L, 0, 0, 0)) return false;
			return true;
		}
	}
