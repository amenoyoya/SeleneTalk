...
/* Setup lexer state. */
int lj_lex_setup_lua(lua_State *L, LexState *ls)
{
  int header = 0;
  ls->L = L;
  ls->fs = NULL;
...
/* Cleanup lexer state. */
void lj_lex_cleanup_lua(lua_State *L, LexState *ls)
{
  global_State *g = G(L);
...
void lj_lex_next_lua(LexState *ls)
{
...
LexToken lj_lex_lookahead_lua(LexState *ls)
{
...
const char *lj_lex_token2str_lua(LexState *ls, LexToken token)
{
  if (token > TK_OFS)
...
void lj_lex_error_lua(LexState *ls, LexToken token, ErrMsg em, va_list args)
{
  const char *tok;
  if (token == 0) {
    tok = NULL;
...
void lj_lex_init_lua(lua_State *L)
{
  uint32_t i;
