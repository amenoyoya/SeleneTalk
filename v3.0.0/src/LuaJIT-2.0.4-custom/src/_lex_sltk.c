...
static int llex(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
  	if(ls->current == '@'){ /* added by yoya. '@'=>'self' */
  		GCstr *s = lj_str_new(ls->L, "self", 4);
  		next(ls);
  		setstrV(ls->L, tv, s);
  		return TK_name;
  	}else if (lj_char_isident(ls->current)) {
  		GCstr *s;
    ...
    case '\f':
      next(ls);
      continue;
    case '/': /* added by yoya. C-like comment */
    	next(ls);
    	if(ls->current == '/'){ /* short comment */
    		while (!currIsNewline(ls) && ls->current != END_OF_STREAM) next(ls);
    		continue;
    		...
    		while(ls->current != END_OF_STREAM){
    			if(ls->current == '\n' || ls->current == '\r'){
    				...
    				next(ls);
    				if(ls->current == '/'){ next(ls); break; } /* end of long comment */
    			}
    			else next(ls);
    		}
    		continue;
    ...
    case '-':
    	next(ls);
    	if(ls->current == '>'){ /* added by yoya. '->' is method call */
    		next(ls); return TK_method;
    	}else return '-';
    case '[': {
    	int sep = skip_sep(ls);
    	if (sep >= 0) {
    ...
    case '!': /* changed by yoya. sltk'!' = lua'~' */
      next(ls);
      if (ls->current != '=') return '!'; else { next(ls); return TK_ne; }
    case ':': /* changed by yoya. ':{table def}' ':goto_label:' */
      next(ls);
      if (ls->current != '{') return ':'; else { next(ls); return TK_tbl; }
    case '"':
    case '\'':
...
/* Setup lexer state. */
int lj_lex_setup_sltk(lua_State *L, LexState *ls)
{
  int header = 0;
  ls->L = L;
  ls->fs = NULL;
...
/* Cleanup lexer state. */
void lj_lex_cleanup_sltk(lua_State *L, LexState *ls)
{
  global_State *g = G(L);
...
void lj_lex_next_sltk(LexState *ls)
{
...
LexToken lj_lex_lookahead_sltk(LexState *ls)
{
...
const char *lj_lex_token2str_sltk(LexState *ls, LexToken token)
{
  if (token > TK_OFS)
...
void lj_lex_error_sltk(LexState *ls, LexToken token, ErrMsg em, va_list args)
{
  const char *tok;
  if (token == 0) {
    tok = NULL;
  ...
  lj_err_lex(ls->L, ls->chunkname, tok, ls->linenumber, em, args);
}

void lj_lex_init_sltk(lua_State *L)
{
  uint32_t i;
