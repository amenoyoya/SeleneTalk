...
#define TKDEF(_, __) \
  ...
  __(bshl, <<) __(bshr, >>) /* Bit shift operations by yoya. */ \
...
GCstr *lj_parse_keepstr_lua(LexState *ls, const char *str, size_t len) /* name changed by yoya. */
{
  /* NOBARRIER: the key is new or kept alive. */
  lua_State *L = ls->L;
...
void lj_parse_keepcdata_lua(LexState *ls, TValue *tv, GCcdata *cd) /* name changed by yoya. */
{
  /* NOBARRIER: the key is new or kept alive. */
  lua_State *L = ls->L;
  setcdataV(L, tv, cd);
...
#define var_new_lit(ls, n, v) \
  var_new(ls, (n), lj_parse_keepstr_lua(ls, "" v, sizeof(v)-1)) /* name changed by yoya. */
...
GCproto *lj_parse_lua(LexState *ls) /* name changed by yoya. */
{
  FuncState fs;
  FuncScope bl;
  GCproto *pt;
