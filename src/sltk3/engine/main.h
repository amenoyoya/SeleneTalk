...
namespace Sltk{
	...
	namespace Engine{
		...
		// Sltk.Engine.errorHandler(errorMessage) を実行する
		...
		/* ScriptFileReader取得 */
		// Sltk.Engine.readerを呼び出す
		FileReader *getFileReader(lua_State *L);
		
		/* Lua/SeleneTalkスクリプトバッファーをロード */
		...
		// mode: 実行モード 1=SeleneTalk, 0=Lua, 2以上=バイトコード
		// 戻り値: -1=ファイルが存在しない, 1=成功,
		//          0=スクリプトロード失敗(Luaスタックの一番上にエラーメッセージ)
		s8 loadScriptFile(lua_State *L, FileReader *pReader, u8 mode=1);
		
		/* メインスクリプトをロード */
		// path: file or directory (暗号化アーカイブなら復号化して実行)
		//       ディレクトリかアーカイブなら内部のMain.sltk(.lua, .sym)をロード
