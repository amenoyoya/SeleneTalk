...
namespace Sltk{
	namespace Win32{
		...
		string selectDirectory(const string &title, u32 len, const string &init_dir, u32 flag, u32 parent){
			wstring ret;
			BROWSEINFO binfo;
			LPITEMIDLIST item = nullptr;
			string cdir = System::getCurrentDirectory();	// カレントディレクトリを取得しておく
			wstring caption = Encoding::utf8ToWideString(title), init = Encoding::utf8ToWideString(init_dir);
			...
			binfo.lpszTitle = caption.c_str();
			binfo.ulFlags = flag;
			binfo.lpfn = &getOpenDirectoryProc;
			binfo.lParam = (LPARAM)init.c_str();
			binfo.iImage = 0;
		...
		// ファイル選択ダイアログ
		string selectFile(const string &openmode, const string &title_, const string &filter_,
			const string &init_dir, u32 flag, u32 parent)
		{
			OPENFILENAME ofn;
			string cdir = System::getCurrentDirectory();	// カレントディレクトリを取得しておく
			...
			ofn.Flags = flag;
			if(openmode == "open") GetOpenFileName(&ofn);
			else if(openmode == "save") GetSaveFileName(&ofn);
