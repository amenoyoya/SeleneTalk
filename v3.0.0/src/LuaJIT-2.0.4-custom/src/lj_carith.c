...
static int carith_int64(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
  if (ctype_isnum(ca->ct[0]->info) && ca->ct[0]->size <= 8 &&
    ...
    setcdataV(L, L->top-1, cd);
    switch (mm) {
    case MM_band: *up = u0 &  u1; break; /* added by yoya. */
    case MM_bor:  *up = u0 |  u1; break; /* added by yoya. */
    case MM_bxor: *up = u0 ^  u1; break; /* added by yoya. */
    case MM_bshl: *up = u0 << u1; break; /* added by yoya. */
    case MM_bshr: *up = u0 >> u1; break; /* added by yoya. */
    case MM_add: *up = u0 + u1; break;
    case MM_sub: *up = u0 - u1; break;
    case MM_mul: *up = u0 * u1; break;
    case MM_div: case MM_idiv: /* changed by yoya. */
      if (id == CTID_INT64)
