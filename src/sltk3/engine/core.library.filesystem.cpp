...
static int writeFile(lua_State *L){
	...
	if(!tolua_isusertype(L, 1, "Sltk::File", 0, &tolua_err) || !tolua_isstring(L, 2, 0, &tolua_err)){
		tolua_error(L, "#ferror in function 'Sltk::File::write'.", &tolua_err);
		return 0;
	}
	File *self = (File*)tolua_tousertype(L, 1, 0);
	if(!self){
		tolua_error(L, "invalid 'self' in function 'Sltk::File::write'", nullptr);
		return 0;
	}
	size_t size;
...
/* Sltk.File.read, Sltk.ZipFileReader.read, Sltk.FileReader.read */
// (u32 size, u32 n=1) -> string binary
#define DEF_READ(File) \
static int read##File(lua_State *L){\
	tolua_Error tolua_err;\
	if(!tolua_isusertype(L, 1, "Sltk::"#File, 0, &tolua_err) || !tolua_isnumber(L, 2, 0, &tolua_err)){\
		tolua_error(L, "#ferror in function 'Sltk::"#File"::read'.", &tolua_err);\
		return 0;\
	}\
	File *self = (File *)tolua_tousertype(L, 1, 0);\
	if(!self){\
		tolua_error(L, "invalid 'self' in function 'Sltk::"#File"::read'.", nullptr);\
		return 0;\
	}\
...
static int appendZipArchiver(lua_State *L){
	tolua_Error tolua_err;
	if(!tolua_isusertype(L, 1, "Sltk::ZipArchiver", 0, &tolua_err)
		|| !tolua_isstring(L, 2, 0, &tolua_err) || !tolua_isstring(L, 3, 0, &tolua_err))
	{
		tolua_error(L, "#ferror in function 'Sltk::ZipArchiver::append'.", &tolua_err);
		return 0;
	}
	ZipArchiver *self = (ZipArchiver*)tolua_tousertype(L, 1, 0);
	if(!self){
		tolua_error(L, "invalid 'self' in function 'Sltk::ZipArchiver::append'", nullptr);
		return 0;
	}
	size_t size;
...
/* Sltk.ZipArchiver.readCurrentFile */
// (u32 size) -> string binary
static int readZipArchiverCurrentFile(lua_State *L){
	tolua_Error tolua_err;
	if(!tolua_isusertype(L, 1, "Sltk::ZipArchiver", 0, &tolua_err) || !tolua_isnumber(L, 2, 0, &tolua_err)) {
		tolua_error(L, "#ferror in function 'Sltk::ZipArchiver::readCurrentFile'.", &tolua_err);
		return 0;
	}
	ZipArchiver *self = (ZipArchiver*)tolua_tousertype(L, 1, 0);
	if(!self){
		tolua_error(L, "invalid 'self' in function 'Sltk::ZipArchiver::readCurrentFile'", nullptr);
		return 0;
	}
...
static int enumerateFiles(lua_State *L){
	...
	if(!tolua_isstring(L, 1, 0, &tolua_err)){
		tolua_error(L, "#ferror in function 'Sltk::enumerateFiles'.", &tolua_err);
		return 0;
	}
