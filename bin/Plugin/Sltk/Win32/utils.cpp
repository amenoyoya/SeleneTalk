...
namespace Sltk{
	namespace Win32{
		...
		bool Process::open(const string &pName){
			...
			if(Process32First(handle, &p32)){
				do{
					if(isSameString(Encoding::toUTF8(p32.szExeFile), pName)){
						CloseHandle(handle);
		...
		u32 Process::getModule(const string &name){
			...
			for(u32 i = 0; i < size; ++i){
				if(isSameString(name, getModuleNameFromHandle((u32)hModules[i]))) return (u32)hModules[i];
			}
			return 0;
		}
		...
		u32 Registry::getValue(void *data, u32 *pSize, const string &name){
			u32 type;
			wstring wname = Encoding::utf8ToWideString(name);
			
			RegQueryValueEx(regKey, wname == L""? nullptr: wname.c_str(), nullptr, &type, nullptr, pSize);
			if(type == REG_SZ){ // 文字列型の場合
				wstring buf;
				string res;
				
				buf.resize(*pSize + 1);
				RegQueryValueEx(regKey, wname == L""? nullptr: wname.c_str(), nullptr, &type, (LPBYTE)buf.c_str(), pSize);
				res = Encoding::toUTF8(buf);
				*pSize = res.size();
				if(!data) return type;
				memcpy(data, res.c_str(), *pSize);
			}else{ // 文字列型でない場合
				if(!data) return type;
				RegQueryValueEx(regKey, wname == L""? nullptr: wname.c_str(), nullptr, &type, (LPBYTE)data, pSize);
			}
			return type;
		}
		
		bool Registry::setValue(const string &name, u32 type, const char *data, u32 size){
			if(type == REG_SZ){
				wstring val = Encoding::utf8ToWideString(data);
				return ERROR_SUCCESS == RegSetValueEx(regKey, name == ""? nullptr: Encoding::utf8ToWideString(name).c_str(),
					0, REG_SZ, (const LPBYTE)val.c_str(), val.size() * sizeof(wchar_t));
			}
			return ERROR_SUCCESS == RegSetValueEx(regKey, name == ""? nullptr: Encoding::utf8ToWideString(name).c_str(),
				0, type, (const LPBYTE)data, size);
		}
		...
		/*** class ColorDialog ***/
		void ColorDialog::setCustomColors(u32 colors[16]){
			for(u32 i = 0; i < 16; ++i){
				// Win32APIの色コードはBGRの順に並んでいる
				customColors[i] = RGB(colors[i] >> 16, ((WORD)colors[i]) >> 8, colors[i]);
			}
		}
		
		bool ColorDialog::show(u32 parent){
			CHOOSECOLOR cc;
			
			ZeroMemory(&cc, sizeof(cc));
			cc.lStructSize = sizeof(cc);
			cc.hwndOwner = (HWND)parent;
			cc.rgbResult = RGB(color >> 16, ((WORD)color) >> 8, color);
			cc.lpCustColors = customColors;
			cc.Flags = CC_RGBINIT;
			if(!ChooseColor(&cc)) return false;
			// BGR -> RGB に変換
			color = ((GetRValue(cc.rgbResult)&0xff)<<16)|((GetGValue(cc.rgbResult)&0xff)<<8)|(GetBValue(cc.rgbResult)&0xff);
			return true;
		}
		
		
		/*** class FontDialog ***/
		/*bool FontDialog::show(const string &initFontName, u32 initFontSize, u32 parent){
			LOGFONT lf;
			CHOOSEFONT cf;
			
			ZeroMemory(&lf, sizeof(lf));
			wcscpy(lf.lfFaceName, Encoding::utf8ToWideString(initFontName).c_str());
			lf.lfHeight = initFontSize;
			
			ZeroMemory(&cf, sizeof(cf));
			cf.lStructSize = sizeof(cf);
			cf.hwndOwner = (HWND)parent;
			cf.lpLogFont = &lf;
			cf.Flags = CF_INITTOLOGFONTSTRUCT;
			if(!ChooseFont(&cf)) return false;
			fontName = Encoding::toUTF8(lf.lfFaceName);
