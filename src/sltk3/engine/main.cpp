	...
	namespace Engine{
		...
		void processError(lua_State *L){
			...
			if(lua_type(L, -1) == LUA_TFUNCTION){
				...
				if(0 != lua_pcall(L, 1, 0, 0)) defErrorHandler(string("[Sltk::Engine::errorHandler]") + lua_tostring(L, -1) + "\n" + err);
				lua_pop(L, 3);
			}else{
				defErrorHandler(err);
				lua_pop(L, 4);
		...
		FileReader *getFileReader(lua_State *L){
			...
			if(tolua_isusertype(L, -1, "Sltk::FileReader", 0, &tolua_err)) pReader = (FileReader*)tolua_tousertype(L, -1, 0);
			lua_pop(L, 3);
			return pReader;
		}
		...
		s8 load(lua_State *L, const string &path, FileReader *_pReader){
			...
			string mainFiles[3] = { "Main.sltk", "Main.lua", "Main.sym" };
			u8 modes[3] = { 1, 0, 2 };
			...
			if(Path::isFile(path)){
				...
				// スクリプトファイルロード
				if(mode < 3 && pReader->openFile(path)){
