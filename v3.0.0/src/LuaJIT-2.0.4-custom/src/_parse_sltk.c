/*
** SeleneTalk parser (source code -> bytecode).
...
/* SeleneTalk lexer tokens. */
#define TKDEF(_, __) \
  _(and) _(break) _(def) _(else) _(elif) _(false) _(for) _(goto) _(if) _(in) \
  _(local) _(nil) _(or) _(repeat) _(return) _(true) _(until) _(while) \
  __(bshl, <<) __(bshr, >>) __(method, ->) __(tbl, :{) /*}*/ \
...
/* added by yoya. */
#define TK_function TK_def
#define TK_elseif TK_elif
#define TK_not '!'
#define TK_label ':'
#define TK_then '{'
#define TK_do '{'
...
typedef enum BinOpr {
  OPR_BAND, OPR_BOR, OPR_BXOR, OPR_BSHL, OPR_BSHR, /* Bit Operations by yoya. */
  OPR_IDIV, /* Integer division by yoya. */
  OPR_ADD, OPR_SUB, OPR_MUL, OPR_DIV, OPR_MOD, OPR_POW,  /* ORDER ARITH */
  OPR_CONCAT, OPR_NE, OPR_EQ, OPR_LT, OPR_GE, OPR_LE, OPR_GT,
...
GCstr *lj_parse_keepstr_sltk(LexState *ls, const char *str, size_t len) /* name changed by yoya. */
{
  /* NOBARRIER: the key is new or kept alive. */
  lua_State *L = ls->L;
...
void lj_parse_keepcdata_sltk(LexState *ls, TValue *tv, GCcdata *cd) /* name changed by yoya. */
{
  /* NOBARRIER: the key is new or kept alive. */
  lua_State *L = ls->L;
  setcdataV(L, tv, cd);
...
static int foldarith(BinOpr opr, ExpDesc *e1, ExpDesc *e2)
{
  ...
  if ((int)opr < OPR_ADD) n = lj_vm_foldarith(expr_numberV(e1), expr_numberV(e2), (int)opr+20); /* added by yoya. */
  else n = lj_vm_foldarith(expr_numberV(e1), expr_numberV(e2), (int)opr-OPR_ADD);
  setnumV(&o, n);
...
static void bcemit_arith(FuncState *fs, BinOpr opr, ExpDesc *e1, ExpDesc *e2)
{
  ...
  }else if (opr < OPR_ADD) { /* added by yoya. */
    op = BC_BAND + opr; 
    rc = expr_toanyreg(fs, e2);
    rb = expr_toanyreg(fs, e1);
  }else {
...
#define var_new_lit(ls, n, v) \
  var_new(ls, (n), lj_parse_keepstr_sltk(ls, "" v, sizeof(v)-1)) /* name changed by yoya. */
...
static void expr_table(LexState *ls, ExpDesc *e)
{
  ...
  bcreg_reserve(fs, 1);
  freg++;
  lex_check(ls, /*'{'*/TK_tbl/* changed by yoya. */);
  while (ls->token != '}') {
    ExpDesc key, val;
    vcall = 0;
    if (ls->token == '[') {
  ...
  lex_match(ls, '}', /*'{'*/TK_tbl/* changed by yoya. */, line);
  if (vcall) {
...
static void parse_body(LexState *ls, ExpDesc *e, int needself, BCLine line)
{
  ...
  lex_check(ls, '{'); /* added by yoya. 'def(){...}' */
  parse_chunk(ls);
...
static void parse_args(LexState *ls, ExpDesc *e)
{
  ...
  if (ls->token == '(') {
    ...
    lex_match(ls, ')', '(', line);
  } else if (ls->token == /*'{'*/TK_tbl/* changed by yoya. */) {
    expr_table(ls, &args);
...
static void expr_primary(LexState *ls, ExpDesc *v)
{
  ...
  for (;;) {  /* Parse multiple expression suffixes. */
    if (ls->token == '.') {
      ...
      expr_bracket(ls, &key);
      expr_index(fs, v, &key);
    } else if (ls->token == /*':'*/TK_method/* changed by yoya. */) {
      ExpDesc key;
      lj_lex_next(ls);
      expr_str(ls, &key);
      bcemit_method(fs, v, &key);
      parse_args(ls, v);
    } else if (ls->token == '(' || ls->token == TK_string || ls->token == /*'{'*/TK_tbl/* changed by yoya. */) {
      expr_tonextreg(fs, v);
      parse_args(ls, v);
    } else {
      break;
...
static void expr_simple(LexState *ls, ExpDesc *v)
{
  ...
  case /*'{'*/TK_tbl/* changed by yoya. */:  /* Table constructor. */
    expr_table(ls, v);
    return;
  case TK_function:
    lj_lex_next(ls);
...
{
  switch (tok) {
  case '|':	return OPR_BOR;      /* added by yoya. */
  case '~':	return OPR_BXOR;     /* added by yoya. */
  case '&':	return OPR_BAND;     /* added by yoya. */
  case TK_bshl:	return OPR_BSHL; /* added by yoya. */
  case TK_bshr:	return OPR_BSHR; /* added by yoya. */
  case '$':	return OPR_IDIV;     /* added by yoya. */
  case '+':	return OPR_ADD;
  case '-':	return OPR_SUB;
  case '*':	return OPR_MUL;
  case '/':	return OPR_DIV;
...
} priority[] = { /* changed by yoya. */
  {5 ,5 }, {6 ,6 }, {7 ,7 }, {8 ,8 }, {8 ,8 }, {12,12},		/* BOR BXOR BAND BSHL BSHR IDIV */
  {11,11}, {11,11}, {12,12}, {12,12}, {12,12},				/* ADD SUB MUL DIV MOD */
  {15,14}, {10,9 }, {3 ,3 }, {3 ,3 },						/* POW CONCAT EQ NE */
  {4 ,4 }, {4 ,4 }, {4 ,4 }, {4 ,4 },						/* LT GE GT LE */
  {2 ,2 }, {1 ,1 }											/* AND OR */
};

#define UNARY_PRIORITY		13  /* Priority for unary operators. changed by yoya. */
...
static void parse_func(LexState *ls, BCLine line)
{
  ...
  while (ls->token == '.')  /* Multiple dot-separated fields. */
    expr_field(ls, &v);
  if (ls->token == /*':'*/TK_method/* changed by yoya. */) {  /* Optional colon to signify method call. */
    needself = 1;
    expr_field(ls, &v);
  }
...
{
  switch (token) {
  /*case TK_else: case TK_elseif:*/ /* deleted by yoya. */ case TK_end: case TK_until: case TK_eof:
    return 1;
  default:
    return 0;
  }
...
static void parse_if(LexState *ls, BCLine line)
{
  ...
  flist = parse_then(ls);
  lex_match(ls, TK_end, TK_if, line); /* added by yoya. 'if {...}' */
  while (ls->token == TK_elseif) {  /* Parse multiple 'elseif' blocks. */
    jmp_append(fs, &escapelist, bcemit_jmp(fs));
    jmp_tohere(fs, flist);
    flist = parse_then(ls);
    lex_match(ls, TK_end, TK_elseif, line); /* added by yoya. 'elif {...}' */
  }
  if (ls->token == TK_else) {  /* Parse optional 'else' block. */
    jmp_append(fs, &escapelist, bcemit_jmp(fs));
    jmp_tohere(fs, flist);
    lj_lex_next(ls);  /* Skip 'else'. */
    lex_check(ls, TK_then); /* added by yoya. 'else {...}' */
    parse_block(ls);
    lex_match(ls, TK_end, TK_else, line); /* added by yoya. 'else {...}' */
  } else {
    jmp_append(fs, &escapelist, flist);
  }
  jmp_tohere(fs, escapelist);
  /*lex_match(ls, TK_end, TK_if, line);*/ /* deleted by yoya. */
}
...
GCproto *lj_parse_sltk(LexState *ls) /* name changed by yoya. */
{
  FuncState fs;
  FuncScope bl;
  GCproto *pt;
