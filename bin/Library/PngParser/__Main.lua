--[[
PNG 画像は最初の 8 バイト（シグネチャ）を除いてチャンクという基本要素から成る
---------------
シグネチャ 8 byte
チャンク 1
チャンク 2
...
以上より、以下が最小構成でもっともシンプルな PNG 画像となる
---------------
8 byte		シグネチャ
25 byte		IHDR
N byte		IDAT
...
local deflate = require("deflate.lua")
...
local function pngImage(path, progCallback, memSave)
    ...
    if not stream or readChar(stream, 8) ~= "\137\080\078\071\013\010\026\010" then 
        return false
    end
